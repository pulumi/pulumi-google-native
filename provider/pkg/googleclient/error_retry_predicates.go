...
// Adapted from https://github.com/GoogleCloudPlatform/magic-modules/blob/e9a0e8f3ca4efe711c2db0b12846623822120de0/mmv1/third_party/terraform/utils/error_retry_predicates.go
...
func is409OperationInProgressError(err error) (bool, string) {
	...
	if gerr.Code == 409 && strings.Contains(gerr.Body, "operationInProgress") {
		logging.V(9).Infof("[DEBUG] Dismissed an error as retryable based on error code 409 and error reason 'operationInProgress': %s", err)
...
func isSubnetworkUnreadyError(err error) (bool, string) {
	...
	if gerr.Code == 400 && strings.Contains(gerr.Body, "resourceNotReady") && strings.Contains(gerr.Body, "subnetworks") {
		logging.V(9).Infof("[DEBUG] Dismissed an error as retryable based on error code 400 and error reason 'resourceNotReady' w/ `subnetwork`: %s", err)
...
func is403ReadRequestsForMinuteError(err error) (bool, string) {
	...
	if gerr.Code == 403 && strings.Contains(gerr.Body, "Quota exceeded for quota metric") && strings.Contains(gerr.Body, "Read requests per minute") {
		logging.V(9).Infof("[DEBUG] Dismissed an error as retryable based on error code 403 and error message 'Quota exceeded for quota metric' on metric `Read requests per minute`: %s", err)
...
func isCommonRetryableErrorCode(err error) (bool, string) {
	...
	if gerr.Code == 429 || gerr.Code == 500 || gerr.Code == 502 || gerr.Code == 503 {
		logging.V(9).Infof("[DEBUG] Dismissed an error as retryable based on error code: %s", err)
...
func pubsubTopicProjectNotReady(err error) (bool, string) {
	if gerr, ok := err.(*googleapi.Error); ok {
		if gerr.Code == 400 && strings.Contains(gerr.Body, "retry this operation") {
			logging.V(9).Infof("[DEBUG] Dismissed error as a retryable operation: %s", err)
