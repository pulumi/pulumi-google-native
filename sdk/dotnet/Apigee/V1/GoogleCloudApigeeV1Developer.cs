// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.GoogleCloud.Apigee.V1
{
    /// <summary>
    /// Creates a developer. Once created, the developer can register an app and obtain an API key. At creation time, a developer is set as `active`. To change the developer status, use the SetDeveloperStatus API.
    /// </summary>
    [GoogleCloudResourceType("google-cloud:apigee/v1:GoogleCloudApigeeV1Developer")]
    public partial class GoogleCloudApigeeV1Developer : Pulumi.CustomResource
    {
        /// <summary>
        /// Create a GoogleCloudApigeeV1Developer resource with the given unique name, arguments, and options.
        /// </summary>
        ///
        /// <param name="name">The unique name of the resource</param>
        /// <param name="args">The arguments used to populate this resource's properties</param>
        /// <param name="options">A bag of options that control this resource's behavior</param>
        public GoogleCloudApigeeV1Developer(string name, GoogleCloudApigeeV1DeveloperArgs args, CustomResourceOptions? options = null)
            : base("google-cloud:apigee/v1:GoogleCloudApigeeV1Developer", name, args ?? new GoogleCloudApigeeV1DeveloperArgs(), MakeResourceOptions(options, ""))
        {
        }

        private GoogleCloudApigeeV1Developer(string name, Input<string> id, CustomResourceOptions? options = null)
            : base("google-cloud:apigee/v1:GoogleCloudApigeeV1Developer", name, null, MakeResourceOptions(options, id))
        {
        }

        private static CustomResourceOptions MakeResourceOptions(CustomResourceOptions? options, Input<string>? id)
        {
            var defaultOptions = new CustomResourceOptions
            {
                Version = Utilities.Version,
            };
            var merged = CustomResourceOptions.Merge(defaultOptions, options);
            // Override the ID if one was specified for consistency with other language SDKs.
            merged.Id = id ?? merged.Id;
            return merged;
        }
        /// <summary>
        /// Get an existing GoogleCloudApigeeV1Developer resource's state with the given name, ID, and optional extra
        /// properties used to qualify the lookup.
        /// </summary>
        ///
        /// <param name="name">The unique name of the resulting resource.</param>
        /// <param name="id">The unique provider ID of the resource to lookup.</param>
        /// <param name="options">A bag of options that control this resource's behavior</param>
        public static GoogleCloudApigeeV1Developer Get(string name, Input<string> id, CustomResourceOptions? options = null)
        {
            return new GoogleCloudApigeeV1Developer(name, id, options);
        }
    }

    public sealed class GoogleCloudApigeeV1DeveloperArgs : Pulumi.ResourceArgs
    {
        /// <summary>
        /// Access type.
        /// </summary>
        [Input("accessType")]
        public Input<string>? AccessType { get; set; }

        /// <summary>
        /// Developer app family.
        /// </summary>
        [Input("appFamily")]
        public Input<string>? AppFamily { get; set; }

        [Input("apps")]
        private InputList<string>? _apps;

        /// <summary>
        /// List of apps associated with the developer.
        /// </summary>
        public InputList<string> Apps
        {
            get => _apps ?? (_apps = new InputList<string>());
            set => _apps = value;
        }

        [Input("attributes")]
        private InputList<Inputs.GoogleCloudApigeeV1AttributeArgs>? _attributes;

        /// <summary>
        /// Optional. Developer attributes (name/value pairs). The custom attribute limit is 18.
        /// </summary>
        public InputList<Inputs.GoogleCloudApigeeV1AttributeArgs> Attributes
        {
            get => _attributes ?? (_attributes = new InputList<Inputs.GoogleCloudApigeeV1AttributeArgs>());
            set => _attributes = value;
        }

        [Input("companies")]
        private InputList<string>? _companies;

        /// <summary>
        /// List of companies associated with the developer.
        /// </summary>
        public InputList<string> Companies
        {
            get => _companies ?? (_companies = new InputList<string>());
            set => _companies = value;
        }

        /// <summary>
        /// Output only. Time at which the developer was created in milliseconds since epoch.
        /// </summary>
        [Input("createdAt")]
        public Input<string>? CreatedAt { get; set; }

        /// <summary>
        /// ID of the developer. **Note**: IDs are generated internally by Apigee and are not guaranteed to stay the same over time.
        /// </summary>
        [Input("developerId")]
        public Input<string>? DeveloperId { get; set; }

        /// <summary>
        /// Required. Email address of the developer. This value is used to uniquely identify the developer in Apigee hybrid. Note that the email address has to be in lowercase only.
        /// </summary>
        [Input("email")]
        public Input<string>? Email { get; set; }

        /// <summary>
        /// Required. First name of the developer.
        /// </summary>
        [Input("firstName")]
        public Input<string>? FirstName { get; set; }

        /// <summary>
        /// Output only. Time at which the developer was last modified in milliseconds since epoch.
        /// </summary>
        [Input("lastModifiedAt")]
        public Input<string>? LastModifiedAt { get; set; }

        /// <summary>
        /// Required. Last name of the developer.
        /// </summary>
        [Input("lastName")]
        public Input<string>? LastName { get; set; }

        /// <summary>
        /// Output only. Name of the Apigee organization in which the developer resides.
        /// </summary>
        [Input("organizationName")]
        public Input<string>? OrganizationName { get; set; }

        /// <summary>
        /// Required. Name of the Apigee organization in which the developer is created. Use the following structure in your request: `organizations/{org}`.
        /// </summary>
        [Input("parent", required: true)]
        public Input<string> Parent { get; set; } = null!;

        /// <summary>
        /// Output only. Status of the developer. Valid values are `active` and `inactive`.
        /// </summary>
        [Input("status")]
        public Input<string>? Status { get; set; }

        /// <summary>
        /// Required. User name of the developer. Not used by Apigee hybrid.
        /// </summary>
        [Input("userName")]
        public Input<string>? UserName { get; set; }

        public GoogleCloudApigeeV1DeveloperArgs()
        {
        }
    }
}
