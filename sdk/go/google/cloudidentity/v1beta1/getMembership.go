...
func LookupMembership(ctx *pulumi.Context, args *LookupMembershipArgs, opts ...pulumi.InvokeOption) (*LookupMembershipResult, error) {
	...
	err := ctx.Invoke("google-native:cloudidentity/v1beta1:getMembership", args, &rv, opts...)
	if err != nil {
		return nil, err
	}
	return &rv, nil
...
// Immutable. The `EntityKey` of the member. Either `member_key` or `preferred_member_key` must be set when calling MembershipsService.CreateMembership but not both; both shall be set when returned.
func (o LookupMembershipResultOutput) MemberKey() EntityKeyResponseOutput {
	return o.ApplyT(func(v LookupMembershipResult) EntityKeyResponse { return v.MemberKey }).(EntityKeyResponseOutput)
}
