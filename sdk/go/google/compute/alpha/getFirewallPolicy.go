...
func LookupFirewallPolicy(ctx *pulumi.Context, args *LookupFirewallPolicyArgs, opts ...pulumi.InvokeOption) (*LookupFirewallPolicyResult, error) {
	...
	err := ctx.Invoke("google-native:compute/alpha:getFirewallPolicy", args, &rv, opts...)
	if err != nil {
		return nil, err
	}
	return &rv, nil
...
// A list of packet mirroring rules that belong to this policy.
func (o LookupFirewallPolicyResultOutput) PacketMirroringRules() FirewallPolicyRuleResponseArrayOutput {
	return o.ApplyT(func(v LookupFirewallPolicyResult) []FirewallPolicyRuleResponse { return v.PacketMirroringRules }).(FirewallPolicyRuleResponseArrayOutput)
}
...
// The scope of networks allowed to be associated with the firewall policy. This field can be either GLOBAL_VPC_NETWORK or REGIONAL_VPC_NETWORK. A firewall policy with the VPC scope set to GLOBAL_VPC_NETWORK is allowed to be attached only to global networks. When the VPC scope is set to REGIONAL_VPC_NETWORK the firewall policy is allowed to be attached only to regional networks in the same scope as the firewall policy. Note: if not specified then GLOBAL_VPC_NETWORK will be used.
func (o LookupFirewallPolicyResultOutput) VpcNetworkScope() pulumi.StringOutput {
	return o.ApplyT(func(v LookupFirewallPolicyResult) string { return v.VpcNetworkScope }).(pulumi.StringOutput)
}
