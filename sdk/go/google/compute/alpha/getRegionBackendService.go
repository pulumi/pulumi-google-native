...
type LookupRegionBackendServiceResult struct {
	...
	HaPolicy BackendServiceHAPolicyResponse `pulumi:"haPolicy"`
	...
	UsedBy     []BackendServiceUsedByResponse `pulumi:"usedBy"`
...
// Specifies the canary migration state. Possible values are PREPARE, TEST, and FINALIZE. To begin the migration from EXTERNAL to EXTERNAL_MANAGED, the state must be changed to PREPARE. The state must be changed to FINALIZE before the loadBalancingScheme can be changed to EXTERNAL_MANAGED. Optionally, the TEST state can be used to migrate traffic by percentage using externalManagedMigrationTestingPercentage. Rolling back a migration requires the states to be set in reverse order. So changing the scheme from EXTERNAL_MANAGED to EXTERNAL requires the state to be set to FINALIZE at the same time. Optionally, the TEST state can be used to migrate some traffic back to EXTERNAL or PREPARE can be used to migrate all traffic back to EXTERNAL.
func (o LookupRegionBackendServiceResultOutput) ExternalManagedMigrationState() pulumi.StringOutput {
	return o.ApplyT(func(v LookupRegionBackendServiceResult) string { return v.ExternalManagedMigrationState }).(pulumi.StringOutput)
}

// Determines the fraction of requests that should be processed by the Global external Application Load Balancer. The value of this field must be in the range [0, 100]. Session affinity options will slightly affect this routing behavior, for more details, see: Session Affinity. This value is only used if the loadBalancingScheme in the BackendService is set to EXTERNAL when using the classic Application Load Balancer.
func (o LookupRegionBackendServiceResultOutput) ExternalManagedMigrationTestingPercentage() pulumi.Float64Output {
	return o.ApplyT(func(v LookupRegionBackendServiceResult) float64 { return v.ExternalManagedMigrationTestingPercentage }).(pulumi.Float64Output)
}
...
// Configuring haPolicy is not supported.
func (o LookupRegionBackendServiceResultOutput) HaPolicy() BackendServiceHAPolicyResponseOutput {
	return o.ApplyT(func(v LookupRegionBackendServiceResult) BackendServiceHAPolicyResponse { return v.HaPolicy }).(BackendServiceHAPolicyResponseOutput)
}
...
// Server-defined URL for this resource with the resource id.
func (o LookupRegionBackendServiceResultOutput) SelfLinkWithId() pulumi.StringOutput {
	return o.ApplyT(func(v LookupRegionBackendServiceResult) string { return v.SelfLinkWithId }).(pulumi.StringOutput)
}
...
func (o LookupRegionBackendServiceResultOutput) UsedBy() BackendServiceUsedByResponseArrayOutput {
	return o.ApplyT(func(v LookupRegionBackendServiceResult) []BackendServiceUsedByResponse { return v.UsedBy }).(BackendServiceUsedByResponseArrayOutput)
}

// The network scope of the backends that can be added to the backend service. This field can be either GLOBAL_VPC_NETWORK or REGIONAL_VPC_NETWORK. A backend service with the VPC scope set to GLOBAL_VPC_NETWORK is only allowed to have backends in global VPC networks. When the VPC scope is set to REGIONAL_VPC_NETWORK the backend service is only allowed to have backends in regional networks in the same scope as the backend service. Note: if not specified then GLOBAL_VPC_NETWORK will be used.
func (o LookupRegionBackendServiceResultOutput) VpcNetworkScope() pulumi.StringOutput {
	return o.ApplyT(func(v LookupRegionBackendServiceResult) string { return v.VpcNetworkScope }).(pulumi.StringOutput)
}
