...
type RegionBackendService struct {
	...
	ExternalManagedMigrationState pulumi.StringOutput `pulumi:"externalManagedMigrationState"`
	...
	ExternalManagedMigrationTestingPercentage pulumi.Float64Output `pulumi:"externalManagedMigrationTestingPercentage"`
	...
	HaPolicy BackendServiceHAPolicyResponseOutput `pulumi:"haPolicy"`
...
func NewRegionBackendService(ctx *pulumi.Context,
	...
	err := ctx.RegisterResource("google-native:compute/alpha:RegionBackendService", name, args, &resource, opts...)
	if err != nil {
		return nil, err
	}
	return &resource, nil
...
func GetRegionBackendService(ctx *pulumi.Context,
	...
	var resource RegionBackendService
	err := ctx.ReadResource("google-native:compute/alpha:RegionBackendService", name, id, state, &resource, opts...)
	if err != nil {
		return nil, err
	}
	return &resource, nil
...
type regionBackendServiceArgs struct {
	...
	// Specifies the canary migration state. Possible values are PREPARE, TEST, and FINALIZE. To begin the migration from EXTERNAL to EXTERNAL_MANAGED, the state must be changed to PREPARE. The state must be changed to FINALIZE before the loadBalancingScheme can be changed to EXTERNAL_MANAGED. Optionally, the TEST state can be used to migrate traffic by percentage using externalManagedMigrationTestingPercentage. Rolling back a migration requires the states to be set in reverse order. So changing the scheme from EXTERNAL_MANAGED to EXTERNAL requires the state to be set to FINALIZE at the same time. Optionally, the TEST state can be used to migrate some traffic back to EXTERNAL or PREPARE can be used to migrate all traffic back to EXTERNAL.
	ExternalManagedMigrationState *RegionBackendServiceExternalManagedMigrationState `pulumi:"externalManagedMigrationState"`
	// Determines the fraction of requests that should be processed by the Global external Application Load Balancer. The value of this field must be in the range [0, 100]. Session affinity options will slightly affect this routing behavior, for more details, see: Session Affinity. This value is only used if the loadBalancingScheme in the BackendService is set to EXTERNAL when using the classic Application Load Balancer.
	...
	// Configuring haPolicy is not supported.
	HaPolicy *BackendServiceHAPolicy `pulumi:"haPolicy"`
	// The list of URLs to the healthChecks, httpHealthChecks (legacy), or httpsHealthChecks (legacy) resource for health checking this backend service. Not all backend services support legacy health checks. See Load balancer guide. Currently, at most one health check can be specified for each backend service. Backend services with instance group or zonal NEG backends must have a health check. Backend services with internet or serverless NEG backends must not have a health check.
	...
	// The network scope of the backends that can be added to the backend service. This field can be either GLOBAL_VPC_NETWORK or REGIONAL_VPC_NETWORK. A backend service with the VPC scope set to GLOBAL_VPC_NETWORK is only allowed to have backends in global VPC networks. When the VPC scope is set to REGIONAL_VPC_NETWORK the backend service is only allowed to have backends in regional networks in the same scope as the backend service. Note: if not specified then GLOBAL_VPC_NETWORK will be used.
	VpcNetworkScope *RegionBackendServiceVpcNetworkScope `pulumi:"vpcNetworkScope"`
}
...
type RegionBackendServiceArgs struct {
	...
	// Specifies the canary migration state. Possible values are PREPARE, TEST, and FINALIZE. To begin the migration from EXTERNAL to EXTERNAL_MANAGED, the state must be changed to PREPARE. The state must be changed to FINALIZE before the loadBalancingScheme can be changed to EXTERNAL_MANAGED. Optionally, the TEST state can be used to migrate traffic by percentage using externalManagedMigrationTestingPercentage. Rolling back a migration requires the states to be set in reverse order. So changing the scheme from EXTERNAL_MANAGED to EXTERNAL requires the state to be set to FINALIZE at the same time. Optionally, the TEST state can be used to migrate some traffic back to EXTERNAL or PREPARE can be used to migrate all traffic back to EXTERNAL.
	ExternalManagedMigrationState RegionBackendServiceExternalManagedMigrationStatePtrInput
	// Determines the fraction of requests that should be processed by the Global external Application Load Balancer. The value of this field must be in the range [0, 100]. Session affinity options will slightly affect this routing behavior, for more details, see: Session Affinity. This value is only used if the loadBalancingScheme in the BackendService is set to EXTERNAL when using the classic Application Load Balancer.
	ExternalManagedMigrationTestingPercentage pulumi.Float64PtrInput
	// Requires at least one backend instance group to be defined as a backup (failover) backend. For load balancers that have configurable failover: [Internal passthrough Network Load Balancers](https://cloud.google.com/load-balancing/docs/internal/failover-overview) and [external passthrough Network Load Balancers](https://cloud.google.com/load-balancing/docs/network/networklb-failover-overview).
	FailoverPolicy BackendServiceFailoverPolicyPtrInput
	// Configuring haPolicy is not supported.
	HaPolicy BackendServiceHAPolicyPtrInput
	// The list of URLs to the healthChecks, httpHealthChecks (legacy), or httpsHealthChecks (legacy) resource for health checking this backend service. Not all backend services support legacy health checks. See Load balancer guide. Currently, at most one health check can be specified for each backend service. Backend services with instance group or zonal NEG backends must have a health check. Backend services with internet or serverless NEG backends must not have a health check.
	...
	// The network scope of the backends that can be added to the backend service. This field can be either GLOBAL_VPC_NETWORK or REGIONAL_VPC_NETWORK. A backend service with the VPC scope set to GLOBAL_VPC_NETWORK is only allowed to have backends in global VPC networks. When the VPC scope is set to REGIONAL_VPC_NETWORK the backend service is only allowed to have backends in regional networks in the same scope as the backend service. Note: if not specified then GLOBAL_VPC_NETWORK will be used.
	VpcNetworkScope RegionBackendServiceVpcNetworkScopePtrInput
}
...
// Specifies the canary migration state. Possible values are PREPARE, TEST, and FINALIZE. To begin the migration from EXTERNAL to EXTERNAL_MANAGED, the state must be changed to PREPARE. The state must be changed to FINALIZE before the loadBalancingScheme can be changed to EXTERNAL_MANAGED. Optionally, the TEST state can be used to migrate traffic by percentage using externalManagedMigrationTestingPercentage. Rolling back a migration requires the states to be set in reverse order. So changing the scheme from EXTERNAL_MANAGED to EXTERNAL requires the state to be set to FINALIZE at the same time. Optionally, the TEST state can be used to migrate some traffic back to EXTERNAL or PREPARE can be used to migrate all traffic back to EXTERNAL.
func (o RegionBackendServiceOutput) ExternalManagedMigrationState() pulumi.StringOutput {
	return o.ApplyT(func(v *RegionBackendService) pulumi.StringOutput { return v.ExternalManagedMigrationState }).(pulumi.StringOutput)
}

// Determines the fraction of requests that should be processed by the Global external Application Load Balancer. The value of this field must be in the range [0, 100]. Session affinity options will slightly affect this routing behavior, for more details, see: Session Affinity. This value is only used if the loadBalancingScheme in the BackendService is set to EXTERNAL when using the classic Application Load Balancer.
func (o RegionBackendServiceOutput) ExternalManagedMigrationTestingPercentage() pulumi.Float64Output {
	return o.ApplyT(func(v *RegionBackendService) pulumi.Float64Output { return v.ExternalManagedMigrationTestingPercentage }).(pulumi.Float64Output)
}
...
// Configuring haPolicy is not supported.
func (o RegionBackendServiceOutput) HaPolicy() BackendServiceHAPolicyResponseOutput {
	return o.ApplyT(func(v *RegionBackendService) BackendServiceHAPolicyResponseOutput { return v.HaPolicy }).(BackendServiceHAPolicyResponseOutput)
}
...
// Server-defined URL for this resource with the resource id.
func (o RegionBackendServiceOutput) SelfLinkWithId() pulumi.StringOutput {
	return o.ApplyT(func(v *RegionBackendService) pulumi.StringOutput { return v.SelfLinkWithId }).(pulumi.StringOutput)
}
...
// The network scope of the backends that can be added to the backend service. This field can be either GLOBAL_VPC_NETWORK or REGIONAL_VPC_NETWORK. A backend service with the VPC scope set to GLOBAL_VPC_NETWORK is only allowed to have backends in global VPC networks. When the VPC scope is set to REGIONAL_VPC_NETWORK the backend service is only allowed to have backends in regional networks in the same scope as the backend service. Note: if not specified then GLOBAL_VPC_NETWORK will be used.
func (o RegionBackendServiceOutput) VpcNetworkScope() pulumi.StringOutput {
	return o.ApplyT(func(v *RegionBackendService) pulumi.StringOutput { return v.VpcNetworkScope }).(pulumi.StringOutput)
}
