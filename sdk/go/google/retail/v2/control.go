...
func NewControl(ctx *pulumi.Context,
	...
	err := ctx.RegisterResource("google-native:retail/v2:Control", name, args, &resource, opts...)
	if err != nil {
		return nil, err
	}
	return &resource, nil
...
func GetControl(ctx *pulumi.Context,
	...
	var resource Control
	err := ctx.ReadResource("google-native:retail/v2:Control", name, id, state, &resource, opts...)
	if err != nil {
		return nil, err
	}
	return &resource, nil
...
type controlArgs struct {
	...
	// A rule control - a condition-action pair. Enacts a set action when the condition is triggered. For example: Boost "gShoe" when query full matches "Running Shoes".
	Rule *GoogleCloudRetailV2Rule `pulumi:"rule"`
	// Specifies the use case for the control. Affects what condition fields can be set. Only settable by search controls. Will default to SEARCH_SOLUTION_USE_CASE_SEARCH if not specified. Currently only allow one search_solution_use_case per control.
...
// A rule control - a condition-action pair. Enacts a set action when the condition is triggered. For example: Boost "gShoe" when query full matches "Running Shoes".
func (o ControlOutput) Rule() GoogleCloudRetailV2RuleResponseOutput {
	return o.ApplyT(func(v *Control) GoogleCloudRetailV2RuleResponseOutput { return v.Rule }).(GoogleCloudRetailV2RuleResponseOutput)
}
