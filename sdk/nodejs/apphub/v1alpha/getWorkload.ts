// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "../../types/input";
import * as outputs from "../../types/output";
import * as enums from "../../types/enums";
import * as utilities from "../../utilities";

/**
 * Gets a Workload in an Application.
 */
export function getWorkload(args: GetWorkloadArgs, opts?: pulumi.InvokeOptions): Promise<GetWorkloadResult> {

    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("google-native:apphub/v1alpha:getWorkload", {
        "applicationId": args.applicationId,
        "location": args.location,
        "project": args.project,
        "workloadId": args.workloadId,
    }, opts);
}

export interface GetWorkloadArgs {
    applicationId: string;
    location: string;
    project?: string;
    workloadId: string;
}

export interface GetWorkloadResult {
    /**
     * Optional. Consumer provided attributes.
     */
    readonly attributes: outputs.apphub.v1alpha.AttributesResponse;
    /**
     * Create time.
     */
    readonly createTime: string;
    /**
     * Optional. User-defined description of a Workload. Can have a maximum length of 2048 characters.
     */
    readonly description: string;
    /**
     * Immutable. The resource name of the original discovered workload.
     */
    readonly discoveredWorkload: string;
    /**
     * Optional. User-defined name for the Workload. Can have a maximum length of 63 characters.
     */
    readonly displayName: string;
    /**
     * Identifier. The resource name of the Workload. Format: "projects/{host-project-id}/locations/{location}/applications/{application-id}/workloads/{workload-id}"
     */
    readonly name: string;
    /**
     * Workload state.
     */
    readonly state: string;
    /**
     * A universally unique identifier (UUID) for the `Workload` in the UUID4 format.
     */
    readonly uid: string;
    /**
     * Update time.
     */
    readonly updateTime: string;
    /**
     * Properties of an underlying compute resource represented by the Workload. These are immutable.
     */
    readonly workloadProperties: outputs.apphub.v1alpha.WorkloadPropertiesResponse;
    /**
     * Reference of an underlying compute resource represented by the Workload. These are immutable.
     */
    readonly workloadReference: outputs.apphub.v1alpha.WorkloadReferenceResponse;
}
/**
 * Gets a Workload in an Application.
 */
export function getWorkloadOutput(args: GetWorkloadOutputArgs, opts?: pulumi.InvokeOptions): pulumi.Output<GetWorkloadResult> {
    return pulumi.output(args).apply((a: any) => getWorkload(a, opts))
}

export interface GetWorkloadOutputArgs {
    applicationId: pulumi.Input<string>;
    location: pulumi.Input<string>;
    project?: pulumi.Input<string>;
    workloadId: pulumi.Input<string>;
}
