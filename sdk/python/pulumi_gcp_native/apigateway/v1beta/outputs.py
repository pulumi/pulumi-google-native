...
class ApigatewayApiConfigGrpcServiceDefinitionResponse(dict):
    ...
    def __init__(__self__, *,
        ...
        """
        A gRPC service definition.
        :param 'ApigatewayApiConfigFileResponseArgs' file_descriptor_set: Input only. File descriptor set, generated by protoc. To generate, use protoc with imports and source info included. For an example test.proto file, the following command would put the value in a new file named out.pb. $ protoc --include_imports --include_source_info test.proto -o out.pb
        :param Sequence['ApigatewayApiConfigFileResponseArgs'] source: Optional. Uncompiled proto files associated with the descriptor set, used for display purposes (server-side compilation is not supported). These should match the inputs to 'protoc' command used to generate file_descriptor_set.
        """
...
class ApigatewayApiConfigOpenApiDocumentResponse(dict):
    ...
    def __init__(__self__, *,
        ...
        """
        An OpenAPI Specification Document describing an API.
        :param 'ApigatewayApiConfigFileResponseArgs' document: The OpenAPI Specification document file.
        """
...
class ApigatewayAuditConfigResponse(dict):
    ...
    def __init__(__self__, *,
        ...
        """
        Specifies the audit configuration for a service. The configuration determines which permission types are logged, and what identities, if any, are exempted from logging. An AuditConfig must have one or more AuditLogConfigs. If there are AuditConfigs for both `allServices` and a specific service, the union of the two AuditConfigs is used for that service: the log_types specified in each AuditConfig are enabled, and the exempted_members in each AuditLogConfig are exempted. Example Policy with multiple AuditConfigs: { "audit_configs": [ { "service": "allServices", "audit_log_configs": [ { "log_type": "DATA_READ", "exempted_members": [ "user:jose@example.com" ] }, { "log_type": "DATA_WRITE" }, { "log_type": "ADMIN_READ" } ] }, { "service": "sampleservice.googleapis.com", "audit_log_configs": [ { "log_type": "DATA_READ" }, { "log_type": "DATA_WRITE", "exempted_members": [ "user:aliya@example.com" ] } ] } ] } For sampleservice, this policy enables DATA_READ, DATA_WRITE and ADMIN_READ logging. It also exempts jose@example.com from DATA_READ logging, and aliya@example.com from DATA_WRITE logging.
        :param Sequence['ApigatewayAuditLogConfigResponseArgs'] audit_log_configs: The configuration for logging of each type of permission.
...
class ApigatewayGatewayConfigResponse(dict):
    ...
    def __init__(__self__, *,
        ...
        """
        Configuration settings for Gateways.
        :param 'ApigatewayBackendConfigResponseArgs' backend_config: Required. Backend settings that are applied to all backends of the Gateway.
        """
