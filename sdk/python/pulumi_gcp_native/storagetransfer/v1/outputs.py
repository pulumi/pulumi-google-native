...
class AzureBlobStorageDataResponse(dict):
    ...
    def __init__(__self__, *,
        ...
        """
        An AzureBlobStorageData resource can be a data source, but not a data sink. An AzureBlobStorageData resource represents one Azure container. The storage account determines the [Azure endpoint](https://docs.microsoft.com/en-us/azure/storage/common/storage-create-storage-account#storage-account-endpoints). In an AzureBlobStorageData resource, a blobs's name is the [Azure Blob Storage blob's key name](https://docs.microsoft.com/en-us/rest/api/storageservices/naming-and-referencing-containers--blobs--and-metadata#blob-names).
        :param 'AzureCredentialsResponseArgs' azure_credentials: Required. Input only. Credentials used to authenticate API requests to Azure. For information on our data retention policy for user credentials, see [User credentials](/storage-transfer/docs/data-retention#user-credentials).
...
class TransferSpecResponse(dict):
    ...
    def __init__(__self__, *,
        ...
        """
        Configuration for running a transfer.
        :param 'AwsS3DataResponseArgs' aws_s3_data_source: An AWS S3 data source.
        :param 'AzureBlobStorageDataResponseArgs' azure_blob_storage_data_source: An Azure Blob Storage data source.
        :param 'GcsDataResponseArgs' gcs_data_sink: A Cloud Storage data sink.
        :param 'GcsDataResponseArgs' gcs_data_source: A Cloud Storage data source.
        :param 'HttpDataResponseArgs' http_data_source: An HTTP URL data source.
        :param 'ObjectConditionsResponseArgs' object_conditions: Only objects that satisfy these object conditions are included in the set of data source and data sink objects. Object conditions based on objects' "last modification time" do not exclude objects in a data sink.
        :param 'TransferOptionsResponseArgs' transfer_options: If the option delete_objects_unique_in_sink is `true` and time-based object conditions such as 'last modification time' are specified, the request fails with an INVALID_ARGUMENT error.
        """
